...
while ($line = db_read($rlt))
	...
	if ($line['pi_desconto'] > 0)
		{
		$peca .= '<TD align="right">'.number_format($line['pi_vlr_unit'] * ($line['pi_desconto']/100),2).'</TD>';
		} else { $peca .= '<TD>&nbsp;</TD>'; }
	$peca .= '<TD align="right">'.number_format($line['pi_vlr_total'] * (1 - ($line['pi_desconto']/100)),2).'</TD>';
	$peca     .= '<TR><TD colspan="10"><img src="img/nada_preto.gif" width="100%" height="1"></TD></TR>';

	$total = $total + $line['pi_vlr_total'] * (1 - ($line['pi_desconto']/100));
	}

$sql = "select * from contas_receber where cr_status <> 'X' and ";
$sql = $sql . " cr_doc = '".strzero($pedido,7)."'";
$rlt = db_query($sql);
...
while ($line = db_read($rlt))
	...
	if ($status == 'A') {$bg = 'bgcolor="#eaffea" '; $tr = ' <font color="#000000">'; $status = 'Aberto'; $tota = $tota + $line['cr_valor'];}
	...
	$sr .= '<TR><TD colspan="10">Total '.number_format($tot,2).', aberto '.number_format($tota,2).'</TD></TR>';
	echo $cab;
	...
	$cab_ped .= '<TR><TD colspan="6"><B>'.$cliente_fantasia.'</B></TD></TR>';
	$cab_ped .= '<TR><TD class="lt0">razão social</TD></TR>';
	$cab_ped .= '<TR><TD colspan="6"><B>'.$cliente_social.'</B></TD></TR>';
