...
$sql .= "left join usuario on id_us = p_log ";
$sql .= "left join vendedores on p_vendedor = vd_codigo ";
...
if ($line = db_read($rlt))
	...
	$vendedor = $line['vd_nome'];
	$valo = $line['p_valor'];
	$desc = $line['p_desconto'];
	$data = $line['p_data'];
	$hora = $line['p_hora'];
	$ped_log = $line['us_login'];
...
while ($line = db_read($rlt))
	...
	$cab_ped .= '<TR><TD class="lt0">representante</TD></TR>';
	$cab_ped .= '<TR><TD colspan="6"><B>'.$vendedor.'</B></TD></TR>';
	$cab_ped .= '<TR><TD class="lt0">nome cliente</TD></TR>';
	$cab_ped .= '<TR><TD colspan="6"><B>'.$cliente_codigo.' - '.$cliente_nome.'</B></TD></TR>';
	...
	$cab_ped .= '<FONT class="lt5"><nobr><CENTER>'.strzero($pedido,5).'</font><font class="lt0"><BR>'.stodbr($data).' '.$ped_log.'</FONT></fieldset>';
	$cab_ped .= "</TABLE>";
