...
if (strlen($dd[2]) == 0)
	{
	redirect("orcamento_vendedor.php");
	}
...
//	$sql .= " left join feira on o_local = feira_codigo ";
	...
	if ($line = db_read($rlt))
		...
		$vendedor = $line['p_vendedor'];
		$dataped = $line['p_data'];
		$local_venda = $line['feira_nome'];
		$local_codigo = $line['p_local'];		
		}
//////////////////////////////////////////////////// FINANCEIRO 1
...
//////////////////////////////////////////////////// PECAS
	...
	while ($line = db_read($rlt))
		...
		if ($line['p_unidade'] == 'P')
			{
			$esql .= "insert into contas_representante (";
			$esql .= "cr_cliente,cr_valor,cr_venc,";
			$esql .= "cr_tipo,cr_historico,cr_pedido,";
			$esql .= "cr_previsao,cr_parcela,cr_dt_quitacao,";
			$esql .= "cr_status,cr_img,cr_doc, ";
			$esql .= "cr_lastupdate,cr_data,cr_conta, ";
			$esql .= "cr_empresa,cr_valor_original,cr_cc, ";
			$esql .= "cr_peso,cr_ref) values (";
			$esql .= "'".$vendedor."',0,".date("Ymd").",";
			$esql .= "'Q','Pedido ".$orc_nr."','M".$pg."',";
			$esql .= "0,'na',19000101,";
			$esql .= "'A','','M".$pg."',";
			$esql .= date("Ymd").",".date("Ymd").",'00012',";
			$esql .= "1,0,'',";
			$esql .= $line['pi_quan'].",'".$line['p_codigo']."');";
			} else {
			$esql .= "insert into contas_representante (";
			$esql .= "cr_cliente,cr_valor,cr_venc,";
			$esql .= "cr_tipo,cr_historico,cr_pedido,";
			...
			$esql .= "cr_empresa,cr_valor_original,cr_cc, ";
			$esql .= "cr_peso,cr_ref) values (";
			$esql .= "'".$vendedor."','".$line['pi_vlr_total']."',".date("Ymd").",";
			$esql .= "'Q','Pedido ".$orc_nr."','M".$pg."',";
			$esql .= "0,'na',19000101,";
			$esql .= "'A','','M".$pg."',";
			$esql .= date("Ymd").",".date("Ymd").",'00012',";
			$esql .= "1,0,'',";
			$esql .= "0,'".$line['p_codigo']."');";			
			}
		$qta  = $line['pi_quan'];
...
//		$rlt = db_query($sql);
		...
		if (!($line = db_read($rlt)))
			...
			$sql .= "p_lastupdate,p_cliente,p_orcamento) values ('09999'";
			$sql .= ",'TEMP','".$cliente_nome."',".$qta.",";
			$sql .= "'0000',".$tov.",".$descp.",".$dataped.",";
...
//	$tog=0;
	...
	while ($line = db_read($rlt))
		...
		$qta  = $line['pi_quan'];
		$desc = $line['pi_desconto'];
		$vlr_brt = round(10*($line['pi_vlr_unit']*(1-$desc/100)))/10;
		$vlr_brt = ($line['pi_vlr_unit']*(1-$desc/100));
		$xtp = $line['p_unidade'];
		array_push($pecas,array($line['pi_codigo'],$line['pi_quan'],$line['pi_desconto'],$line['pi_vlr_unit'],$line['p_unidade'],$line['pi_vlr_total']));
		...
		$sql1 .= "'".$line['pi_codigo']."','".$line['pi_vlr_unit']."','".$line['pi_vlr_total']."',";
		$sql1 .= "'".$line['pi_desconto']."','".date("Ymd")."','".date("H:i")."',";
		$sql1 .= "'".$user_id."','A','".$orc_nr."',";
		$sql1 .= "'".$line['pi_quan']."');".chr(10).chr(13);
		///////////////////////////////////////// pecas peso
		if ($xtp == 'P')
			{
			array_push($peca_peso,array($line['pi_codigo'],$line['pi_quan']));
			}
		////////////////////////////////////////////////////
		$ip = $_SERVER["REMOTE_ADDR"];
		$sinal = ' - ';
		$sql3 .= "update estoque_produto set p_estoque_1 = p_estoque_1 ".$sinal." ".$line['pi_quan']." ";
		$sql3 .= ", p_estoque_3 = p_estoque_3 - ".$line['pi_quan']." ";
		$sql3 .= " where p_codigo ='".$line['pi_codigo']."'; " .chr(13);
		...
		$sql3 .= $user_id.",'".$line['pi_codigo']."',".$line['pi_quan'].",'".$nr_pedido."', ";
		$sql3 .= "'Q');".chr(13);
...
$sql5 = "update pedido_vendedor set p_status = 'C' where p_orcamento = ".$pg."; ";

$sql = $sql1 . $sql3 . $sql2 . $sql5 . $esql;
//echo $sql;
//echo '<HR>';
//echo $esql;
$rlt = db_query($sql);
